...
function loadNextMatchday() {
  loadDump("nextMatchday.json").then((data) => {
    document.querySelector(".nextMatchday .gastgeber").innerHTML =
      data.gastgeber;
    ...
    document
      .querySelector(".nextMatchday .oskardarten")
      ?.classList.add(data.oskardarten ? "active" : "inactive");
    document
      .querySelector(".nextMatchday .gelddarten")
      ?.classList.add(data.gelddarten ? "active" : "inactive");
  });
}
