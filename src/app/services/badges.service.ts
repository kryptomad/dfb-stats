...
export class BadgesService {
  ...
  readonly badgeDefinitions: BadgeDefinition[] = [
    { name: 'TON-Machine', punkte: 5, beschreibung: 'Meisten TONs am Spieltag', kategorie: 'Score', icon: 'fa-solid fa-square-root-variable' },
    { name: '140-Bomber', punkte: 7, beschreibung: 'Meisten 140 am Spieltag', kategorie: 'Score', icon: 'fa-solid fa-bomb' },
    { name: '180er-Gott', punkte: 10, beschreibung: 'Meisten 180 am Spieltag', kategorie: 'Score', icon: 'fa-solid fa-burst' },
    { name: '26-Legende', punkte: 3, beschreibung: 'Am meisten 26 geworfen', kategorie: 'Score', icon: 'fa-solid fa-fish' },
    { name: 'Checkout-Monster', punkte: 8, beschreibung: '100+ Checkout am Spieltag', kategorie: 'Checkout', icon: 'fa-solid fa-ghost' },
    { name: 'Big Finish', punkte: 8, beschreibung: 'Höchster Checkout am Spieltag', kategorie: 'Checkout', icon: 'fa-solid fa-crosshairs' },
    { name: 'Clutch King', punkte: 8, beschreibung: 'Beste 1-Dart-Checkout-Quote %', kategorie: 'Checkout', icon: 'fa-solid fa-crown' },
    { name: 'Safe Finisher', punkte: 7, beschreibung: 'Beste 2-Dart-Checkout-Quote %', kategorie: 'Checkout', icon: 'fa-solid fa-lock' },
    { name: 'Drama King', punkte: 6, beschreibung: 'Beste 3-Dart-Checkout-Quote %', kategorie: 'Checkout', icon: 'fa-solid fa-mask' },
    { name: 'Fast & Furious', punkte: 8, beschreibung: 'Beste Leg des Spieltags', kategorie: 'Performance', icon: 'fa-solid fa-stopwatch' },
    { name: 'Average King', punkte: 8, beschreibung: 'Höchster 3-Dart Average am Spieltag', kategorie: 'Performance', icon: 'fa-solid fa-percent' },
    { name: 'Power Scorer', punkte: 8, beschreibung: 'Höchster First-9 Average am Spieltag', kategorie: 'Performance', icon: 'fa-solid fa-robot' },
    { name: 'Iron Man', punkte: 5, beschreibung: 'Meisten Legs gespielt', kategorie: 'Performance', icon: 'fa-solid fa-dumbbell' },
    { name: 'Streak Shooter', punkte: 5, beschreibung: '3 Spiele in Folge 3DA > Ø55', kategorie: 'Performance', icon: 'fa-solid fa-trophy' },
    { name: 'Comeback Hero', punkte: 3, beschreibung: 'Sieg nach 0:2 Rückstand', kategorie: 'Performance', icon: 'fa-solid fa-rotate-left' },
    { name: 'Bust-King', punkte: 3, beschreibung: 'Längste Serie an Bust Würfen', kategorie: 'Fun', icon: 'fa-solid fa-trash' },
    { name: 'Kleinvieh', punkte: 3, beschreibung: 'Meisten Würfe unter 26 bei den First 9', kategorie: 'Fun', icon: 'fa-solid fa-poop' },
  ];
  ...
  private init(): void {
    const seasons = [...new Set(this.stats.map((s: any) => s.season as string))].sort();
  ...
  getBadgesForKategorie(kategorie: string): BadgeWithHolder[] {
    return this.badgeDefinitions
      ...
      .map(b => {
        const holder = this.getCurrentHolder(b.name);
        return { ...b, halterId: holder.playerId, count: holder.count };
      });
  }
  ...
  getCurrentHolder(badgeName: string): { playerId: number; count: number } {
    ...
    if (latestAwards.length === 0) return { playerId: 0, count: 0 };

    const holderId = latestAwards[0].playerId;
    const count = this.allAwards.filter(
      a => a.badgeName === badgeName && a.playerId === holderId
    ).length;
    return { playerId: holderId, count };
  }
  ...
  getChipTotals(): LeaderboardEntry[] {
    ...
    for (let md = 1; md <= this.latestMatchday; md++) {
      ...
      for (const award of mdAwards) {
        const def = this.badgeDefinitions.find(b => b.name === award.badgeName);
        if (!def) continue;
        chipMap.set(award.playerId, (chipMap.get(award.playerId) || 0) + def.punkte);
      }
  ...
  private calculateMatchdayBadges(season: string, matchday: number): void {
    ...
    this.awardByPlayerSum(season, matchday, 'TON-Machine', mdStats,
      s => s.score_100 || 0, 1);

    // 140-Bomber: meisten 140er (score_140)
    this.awardByPlayerSum(season, matchday, '140-Bomber', mdStats,
      s => s.score_140 || 0, 1);

    // 180er-Gott: meisten 180er
    this.awardByPlayerSum(season, matchday, '180er-Gott', mdStats,
      s => s.score_180 || 0, 1);

    // Average King: gewichteter Spieltag-Durchschnitt (3-Dart)
    this.awardByWeightedAvg(season, matchday, 'Average King', mdStats,
      s => s.avg_3dart || 0, s => s.darts_thrown || 0);

    // Power Scorer: gewichteter Spieltag-Durchschnitt (First 9)
    this.awardByWeightedAvg(season, matchday, 'Power Scorer', mdStats,
      s => s.avg_first9 || 0, s => s.legs_played || 0);

    // Iron Man: meisten Legs gespielt
    this.awardByPlayerSum(season, matchday, 'Iron Man', mdStats,
      s => s.legs_played || 0);
  ...
  private awardByPlayerSum(
    season: string, matchday: number, badgeName: string,
    mdStats: any[], extractor: (s: any) => number, minValue = 0
  ): void {
  ...
  private calculateStreakShooter(season: string, matchday: number, mdStats: any[]): void {
    ...
    for (const pid of winners) {
      this.allAwards.push({ season, matchday, badgeName: 'Streak Shooter', playerId: pid });
    }
  }
  ...
  private calculateLegsBadges(season: string, matchday: number, mdGames: any[]): void {
    ...
    for (const game of mdGames) {
      ...
      for (const leg of game.legs || []) {
        ...
        if (leg.leg_winner_id === p1 && leg.p1_darts_leg > 0) {
          p1Data.bestWonLegDarts = Math.min(p1Data.bestWonLegDarts, leg.p1_darts_leg);
        }
        if (leg.leg_winner_id === p2 && leg.p2_darts_leg > 0) {
          p2Data.bestWonLegDarts = Math.min(p2Data.bestWonLegDarts, leg.p2_darts_leg);
        }
        ...
        for (const round of rounds) {
          ...
          if (p1Score !== null && p1Score < 0) {
            p1Data.checkouts.push({ value: prevP1Left, darts: Math.abs(p1Score) });
          }
          if (p2Score !== null && p2Score < 0) {
            p2Data.checkouts.push({ value: prevP2Left, darts: Math.abs(p2Score) });
          }

          // Kleinvieh: First 9 (Runden 1-3), Score < 26
          if (round.round >= 1 && round.round <= 3) {
            if (p1Score !== null && p1Score >= 0 && p1Score < 26) p1Data.kleinviehCount++;
            if (p2Score !== null && p2Score >= 0 && p2Score < 26) p2Data.kleinviehCount++;
          }

          // Left-Werte für Checkout-Berechnung tracken
          if (round.p1_left !== null && round.p1_left !== undefined) prevP1Left = round.p1_left;
          if (round.p2_left !== null && round.p2_left !== undefined) prevP2Left = round.p2_left;
        }
    ...
    this.awardFromMap(season, matchday, 'Fast & Furious', playerData,
      d => d.bestWonLegDarts < Infinity ? -d.bestWonLegDarts : -Infinity, 1,
      true); // invertiert: höchster negativer Wert = niedrigste Darts

    // 26-Legende
    this.awardFromMap(season, matchday, '26-Legende', playerData, d => d.count26, 1);

    // Bust-King
    this.awardFromMap(season, matchday, 'Bust-King', playerData, d => d.maxBustStreak, 1);

    // Kleinvieh
    this.awardFromMap(season, matchday, 'Kleinvieh', playerData, d => d.kleinviehCount, 1);

    // Comeback Hero
    this.awardFromMap(season, matchday, 'Comeback Hero', playerData, d => d.comebacks, 1);

    // Checkout-Monster: meisten Checkouts >= 100
    this.awardFromMap(season, matchday, 'Checkout-Monster', playerData,
      d => d.checkouts.filter((c: any) => c.value >= 100).length, 1);

    // Big Finish: höchster einzelner Checkout
    this.awardFromMap(season, matchday, 'Big Finish', playerData,
      d => d.checkouts.length > 0 ? Math.max(...d.checkouts.map((c: any) => c.value)) : 0, 1);
  ...
  private awardFromMap(
    season: string, matchday: number, badgeName: string,
    data: Map<number, any>, extractor: (d: any) => number,
    minValue = 0, _inverted = false
  ): void {
    let best = -Infinity;
    for (const [, d] of data) {
  ...
  private awardCheckoutPct(
    ...
    for (const [pid, d] of data) {
      ...
      const matching = d.checkouts.filter(c => c.darts === targetDarts).length;
